...
try:
    ...
    print("Failed to import built-in modules, maybe your Python version isn't supported. If you think this wrong you can open a issue in https://github.com/bgd31/Discord-spammer/issues .")
    if os.system == "NT":
        time.sleep(100009)
    exit()

if os.name == "nt":
    os.system(f"title Bgd`s project {version}")
else:
    sys.stdout.write(f"\x1b]2; Bgd`s project{version}\x07")
...
log_file = open("bgd`s project-logs.log", "w")
...
default_input_text = f"{Fore.RED}{Style.BRIGHT}bgd`s project 1.0{Fore.BLUE}@python{sys.version_info[0]}.{sys.version_info[1]}.{sys.version_info[2]} {Style.RESET_ALL}$ "
...
def update_check_event():
    ...
    url = "https://raw.githubusercontent.com/bgd31/Discord-spammer/main/revision.txt"
    try:
    ...
    if int(get_revision) >  int(revision):
        ...
        main_file = requests.get("https://raw.githubusercontent.com/bgd31/Discord-spammer/main/main.py").text
        print("DONE")
...
if __name__ == "__main__":
    # Uncomment this to prevent Bgd`s project from checking updates.
